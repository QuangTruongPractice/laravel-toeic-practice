<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Thêm câu hỏi:') }} {{ $exam->title }} - Part {{ $part->part_number }}
            </h2>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.exams.question-groups.store', [$exam, $part]) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg space-y-4">
                            ...
                            @if(in_array($part->part_number, [3, 4, 6, 7]))
                                <div>
                                    ...
                                    <textarea name="passage" id="passage" rows="6" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"></textarea>
                                </div>
                            @endif
                        </div>
    ...
    <script>
        function questionForm() {
            // Xác định số lượng câu hỏi ban đầu dựa theo Part để tiện UX
            let initialCount = 1;
            const partNum = {{ $part->part_number }};
            if (partNum === 3 || partNum === 4) initialCount = 3;
            if (partNum === 6) initialCount = 4;

            // Đáp án mặc định cho từng part (Part 2 có 3 lựa chọn A, B, C)
